...
from ansible_collections.check_point.gaia.plugins.modules import cp_gaia_radius_server

EXPECTED_RESULT = {
    'default_shell': 'cli',
    'nas_ip': '',
    'servers': [{'address': '1.2.1.2', 'port': 56, 'priority': 3, 'timeout': 1}],
    'super_user_uid': '96'
}
...
function_path = 'ansible_collections.check_point.gaia.plugins.modules.cp_gaia_radius_server.chkp_api_call'
api_call_object = 'radius'


class TestCheckpointRadiusServer(object):
    ...
    def run_wrapper(self, mocker, connection_mock, changed):
        ...
        assert EXPECTED_RESULT.items() == result['radius'].items()
