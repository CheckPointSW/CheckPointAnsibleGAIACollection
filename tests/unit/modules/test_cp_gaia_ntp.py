...
from ansible_collections.check_point.gaia.plugins.modules import cp_gaia_ntp

EXPECTED_RESULT = {
    'enabled': False,
    'servers': [{'address': '1.1.1.1', 'status': False, 'type': 'primary', 'version': '1'}]
}
...
function_path = 'ansible_collections.check_point.gaia.plugins.modules.cp_gaia_ntp.chkp_api_call'
api_call_object = 'ntp'


class TestCheckpointNtp(object):
    module = cp_gaia_ntp
    ...
    def run_wrapper(self, mocker, connection_mock, changed):
        ...
        assert EXPECTED_RESULT.items() == result['ntp'].items()
